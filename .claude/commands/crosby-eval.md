...
2. **Read the current tool definitions** from the chat route:
   ```
   Read src/app/api/chat/route.ts - scan for all tool definitions (look for tool name strings and their input schemas)
   ```
...
4. **Check for prior eval findings** - look in Crosby's memory and project context for past eval session summaries so you can catch repeat offenders:
   ```sql
...
5. **Scan for new crons/background processes** (these change as features get added):
   ```bash
...
2. **Pull the recent exchange** including tool call data:
   ```sql
...
**The Big Two** (read these every eval session - they define what Crosby does):
| What | Where |
|------|-------|
| System prompt & all behavior rules | `src/lib/system-prompt.ts` |
...
- **Don't forget the new tools:** `ask_structured_question` and `quick_confirm` were recently added. Check if Crosby is using them when it should be (multi-choice questions, yes/no confirmations) and NOT using them when it shouldn't (open-ended questions, mid-conversation flow).
